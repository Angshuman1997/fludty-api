...
@token_required
def all_drinks():
    data = mongo.db.drinks.find({}, {'_id': 1, 'name': 1, 'image': 1}).skip(int(request.form['offset'])).limit(10)
    ...
    if request.form['login_type'] == "login":
        ...
        if fetch_data:
            if fetch_data['password'] == request.form['password']:
                ...
                token = jwt.encode({'user': request.form['userid'],'expiration': str(datetime.now() + timedelta(seconds=7200)).split(".")[0]},app.config['SECRET_KEY'])
